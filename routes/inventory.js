...
router.route('/').get((req, res) => {
    Inventory.find()
        .then((books) => {
            res.render('inventoryPage', { books });
        })
        .catch((error) => {
...
router.route('/add').post((req, res) => {
    ...
    newBook.save()
        .then(() => {
            ...
            res.redirect(`/owner/${req.params.userID}/inventory`)
        })
        .catch(err => console.log(err.message));
});

//Updating a book.
router.route('/update/:bookID').post((req, res) => {
...
//Deleting a book.
router.route('/delete/:bookID').post((req, res) => {
